...
class WickCanvas {
  ...
  preloadAssets(wickProject, options) {
    WickCanvas._onAssetsLoaded = options.onload;
  ...
  _preloadSounds(wickProject, options) {
    var self = this;
    WickCanvas._totalAssetCount = wickProject.assets.length;
    wickProject.assets.filter(asset => {
      return asset instanceof Wick.SoundAsset;
    }).forEach(asset => {
      if (!WickCanvas._soundsCache[asset.uuid]) {
        WickCanvas._soundsCache[asset.uuid] = new Howl({
          src: [asset.src]
        });
        WickCanvas._soundsCache[asset.uuid].onload = self._checkAllSoundsAreLoaded(asset.uuid);
      }
    });
  }

  _checkAllSoundsAreLoaded(currentUUID) {
    WickCanvas._soundUUIDsLoaded.push(currentUUID);

    if (WickCanvas._soundUUIDsLoaded.length === WickCanvas._totalAssetCount) {
      WickCanvas._onAssetsLoaded && WickCanvas._onAssetsLoaded();
    }
  }
...
WickCanvas.Frame = class {
  ...
  _updateSound(wickFrame, options) {
    var howl = WickCanvas._soundsCache[wickFrame._soundAssetUUID];
